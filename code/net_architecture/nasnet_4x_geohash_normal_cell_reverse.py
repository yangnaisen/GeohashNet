...
def transition_up(x, p, num_filter, block_id, weight_decay=0.0):
    ...
    x_1 = UpSampling2D(size=(2, 2), interpolation='nearest')(x_1)
    return x_1
...
def NAS_U_Net(input_tensor, number_of_class, weight_decay=0.0):
    ...
    p = transition_up(
        normal_cell_16_x, p, filters , '16_to_8', weight_decay=weight_decay)

    p = transition_up(
        normal_cell_8_x, p, filters // filter_multiplier, '8_to_4', weight_decay=weight_decay)

    x, p = _normal_a_cell(normal_cell_4_x, p, filters // (filter_multiplier ** 2), block_id='final_res_4')    
    x = Activation('relu')(x)
