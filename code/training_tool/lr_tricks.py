...
class CyclicCosineRestart(keras.callbacks.Callback):
    def __init__(self, lr_min=1e-6, lr_max=5e-5, 
                  number_of_batches=120,number_of_epochs=50,
                  use_warmup=True):
    ...
    def warmup_lr(self):
        lr_warm = np.linspace(0, self.lr_max, 5*self.number_of_batches)
        self.warmup_lr_iter = itertools.cycle(lr_warm.tolist())
        
    def cosine_lr(self):
        T_current = np.arange(self.number_of_epochs*self.number_of_batches)
        lr_current = self.lr_min + 0.5 * (self.lr_max - self.lr_min) * (
            1 + np.cos((T_current /(self.number_of_epochs*self.number_of_batches)) * np.pi))
