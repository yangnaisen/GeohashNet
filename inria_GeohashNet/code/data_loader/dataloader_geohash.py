...
from .data_loader_in_memory import InriaDataLoaderInMemory
from .geohash.geohash_coding import encode, get_top_left_latlng


class InriaDataLoaderGeohash(InriaDataLoaderInMemory):
    def __init__(self,
                 x_set_dir,
                 y_set_dir,
                 patch_size,
        ...
        super(InriaDataLoaderGeohash, self).__init__(
            x_set_dir,
            y_set_dir,
            patch_size,
            patch_stride,
        ...
        if not (geohash_precision is None):
            for filename in self.images_filenames:
                ...
                lat, lng = get_top_left_latlng(filename)
                geohash_code_list = encode(
                    float(lat), float(lng), precision=self.geohash_precision)

                # The geohash_array can be seen as an image
                # with size of 1*1*len.
                num_code_len = len(geohash_code_list)
                geohash_array = np.array(
                    geohash_code_list, dtype='float32').reshape((1, 1,
                                                                 num_code_len))
                self.geohash_codes.append(geohash_array)

    def get_batch_geohash(self, batch_index):
        batch_patch_idx = self.patches_index[batch_index * self.batch_size:(
