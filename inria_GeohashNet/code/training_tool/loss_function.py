...
def dice_coef_loss(y_true, y_pred, epsilon=1.0):
    ...
    channel_dice_loss = tf.clip_by_value(channel_dice_loss, _epsilon, 1 - _epsilon)
...
def lovasz_softmax_loss(batch_y):
    ...
    for c in range(number_of_class):
        errors_sorted = tf.cond(
            K.any(forground_counter[c]),
            lambda: tf.nn.top_k(positive_errors[:, c], k=forground_counter[c])[0],
            lambda: tf.convert_to_tensor(np.array([0.0], dtype='float32')))
...
def hard_example_mining(batch_y):
    ...
    channel_dice_coefficient = tf.divide(numerator, tf.add(denominator, epsilon))
